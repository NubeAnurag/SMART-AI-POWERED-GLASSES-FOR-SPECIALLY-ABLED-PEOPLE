...
class VoiceEnabledHumanDetectionSystem:
    def __init__(self):
        self.known_faces = {}
        ...
        self.CHANNELS = 1
        self.RATE = 44100
        self.RECORD_SECONDS = 5  # 5 seconds for voice input
    ...
    def record_voice(self, person_name):
        """Record voice input for a person"""
        print(f"🎤 Recording voice for {person_name}...")
        print("💬 Please speak your details (name, age, relationship, etc.)")
        ...
        voice_filename = f"voice_recordings/{person_name}_{timestamp}.wav"
    ...
    def start_detection(self):
        ...
        print("Voice-Enabled Human Detection System Started!")
        print("Press 'q' to quit, 's' to save face, 'v' to record voice, 'h' for help")
        ...
        while True:
            ...
            if is_human:
                ...
                for i, (face_encoding, face_location) in enumerate(zip(face_encodings, face_locations_recognition)):
                    ...
                    if recognized_name:
                        ...
                        voice_text = "🎤 Voice available" if person_data.get("voice_file") else "🎤 No voice recorded"
                    else:
                    ...
                    if voice_text:
                        cv2.putText(frame, voice_text, (left, y_position), 
                                  cv2.FONT_HERSHEY_SIMPLEX, DISPLAY_CONFIG["font_scale"], 
                                  (0, 255, 255), DISPLAY_CONFIG["font_thickness"])  # Yellow color for voice
            else:
                # No human detected
            ...
            if key == ord('q'):
                ...
                self.record_voice_for_current_face(frame, face_encodings, face_locations_recognition)
            elif key == ord('p'):
                self.play_voice_for_current_face(frame, face_encodings, face_locations_recognition)
            elif key == ord('h'):
                self.show_help()
    ...
    def save_current_face(self, frame, face_encodings, face_locations):
        ...
        print("💡 Press 'v' to record voice for this person")
    
    def record_voice_for_current_face(self, frame, face_encodings, face_locations):
        """Record voice for the currently detected face"""
        if not face_encodings:
        ...
        if recognized_name:
            # Record voice for recognized person
            voice_file = self.record_voice(recognized_name)
            
            # Update the person's voice file
            self.known_faces[recognized_name]["voice_file"] = voice_file
            
            # Save voice mappings
            self.save_voice_mappings()
            
            print(f"✅ Voice recorded for {recognized_name}")
        else:
            print("❌ Face not recognized. Save the face first with 's' key")
    
    def play_voice_for_current_face(self, frame, face_encodings, face_locations):
        """Play voice for the currently detected face"""
        if not face_encodings:
        ...
        if recognized_name:
            ...
            if voice_file:
                # Play voice in a separate thread to avoid blocking
    ...
    def show_help(self):
        ...
        print("\n" + "="*50)
        print("🎯 VOICE-ENABLED HUMAN DETECTION SYSTEM HELP")
        print("="*50)
        print("📹 Controls:")
        print("   'q' - Quit the application")
        print("   's' - Save current face to database")
        print("   'v' - Record voice for current face")
        print("   'p' - Play voice for current face")
        ...
        print("   • Voice recording and playback")
        print("   • Real-time display with bounding boxes")
        print("\n🎤 Voice Features:")
        print("   • Record personal details via voice")
        print("   • Automatic voice playback on recognition")
        print("   • Store voice files with face encodings")
        print("="*50)
...
def main():
    """Main function to run the voice-enabled human detection system"""
    print("Initializing Voice-Enabled Human Detection and Identification System...")
    ...
    print("- Record and play voice descriptions")
    print("- Display 'I don't know this person' for unrecognized faces")
    
    system = VoiceEnabledHumanDetectionSystem()
